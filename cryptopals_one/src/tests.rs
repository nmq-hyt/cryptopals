...
use crate::utils::{self, functions::{angle_between_vectors, calculate_index_of_coincidence, chi_squared_score, fixed_xor, frequency_hash_table, hex_to_base64}};
...
fn brute_force_single_byte_xor(text:Vec<u8>) ->  u8{
    // originally we used in similarity comparsion betwee index of coincidence
    // that's actually fine on it's own for 1.4, because you're (implicitly) trying to find which of those strings is actually english language
    let mut scorekeeper: BTreeMap<Decimal, u8> = BTreeMap::new();
    // let mut keeperscore: BTreeMap<u8,Decimal> = BTreeMap::new();
    let mut text_as_string: String;
    
    let freqs: Vec<Decimal> = vec![
        // 0–31
        ...
        // 65–90 (A–Z)
        dec!(0.0651738), dec!(0.0124248), dec!(0.0217339), dec!(0.0349835),
        dec!(0.1041442), dec!(0.0197881), dec!(0.0158610), dec!(0.0492888),
        dec!(0.0558094), dec!(0.0009033), dec!(0.0050529), dec!(0.0331490),
        dec!(0.0202124), dec!(0.0564513), dec!(0.0596302), dec!(0.0137645),
        dec!(0.0008606), dec!(0.0497563), dec!(0.0515760), dec!(0.0729357),
        dec!(0.0225134), dec!(0.0082903), dec!(0.0171272), dec!(0.0013692),
...
fn hamming_distance(string_one: &[u8], string_two: &[u8]) -> u32{
   assert_eq!(string_one.len(),string_two.len());

   return string_one
   .into_iter().zip(string_two.iter())
...
fn test_single_byte_xor_cipher() {
    ...
    // should have just done it properly first
...
fn test_hamming_distance_works_as_expected() {
    ...
    fn brute_force_xor_printing(text:Vec<u8>)  {
        for i in 0u8 .. 127 {
    ...
    fn test_break_repeating_key_xor() -> (){
        let path: &Path = Path::new("decoded.txt");
        let bytes: Vec<u8> = fs::read(path).unwrap();

        let mut keysize_scores: Vec<(u32, usize)> = Vec::new();
            for i in 2..40{
            let first_half = &bytes[0 .. (i)];
            let second_half = &bytes[i  .. (i*2)];
            let third_half = &bytes[(i*2)..(i*3)];
            let fourth_half = &bytes[(i*3)..(i*4)];
        ...
        keysize_scores.sort_by(|a, b| a.0.cmp(&b.0));  
        ...
        let decrypt_attempt:Vec<u8> = bytes
        .iter()
        .zip(key
            .iter()
            .cycle()
        ).map(|f| f.0 ^ f.1 ).collect();
    println!("{}", String::from_utf8_lossy(&decrypt_attempt));
    }
}
